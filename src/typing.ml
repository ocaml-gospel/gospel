...
let rec dterm whereami kid ns denv { term_desc; term_loc = loc } : dterm =
  let mk_dterm ~loc dt_node dty =
    { dt_node; dt_dty = Some dty; dt_loc = loc }
  in
  let apply dt1 t2 =
    let dt2 = dterm whereami kid ns denv t2 in
    let dty = dty_fresh () in
  ...
  let gen_app ~loc ls tl =
    ...
    let dtl = List.map (dterm whereami kid ns denv) args in
    let dtyl, dty = specialize_ls ls in
    if ntl < nls then
      let dtyl1, dtyl2 = split_at_i ntl dtyl in
      let dtl = List.map2 dterm_expected dtl dtyl1 in
      let dty =
        List.fold_right
      ...
      let dtl = List.map2 dterm_expected dtl dtyl in
  ...
  let rec unfold_app t1 t2 tl =
    ...
    | _ ->
        let dt1 = dterm whereami kid ns denv t1 in
        map_apply dt1 (t2 :: tl)
  in
          ...
          let rec aux expected defined =
            ...
            | (ls, dty) :: xs, ((q, t) as y) :: ys -> (
                ...
                | 0 ->
                    ...
                    and dt = dterm whereami kid ns denv t in
                    (dterm_expected dt dty :: fields, missing)
                | n when n < 0 ->
  ...
  | Uast.Tnot t ->
      let dt = dterm whereami kid ns denv t in
      dfmla_unify dt;
      mk_dterm ~loc (DTnot dt) (Option.get dt.dt_dty)
  | Uast.Tif (t1, t2, t3) ->
      let dt1 = dterm whereami kid ns denv t1 in
      let dt2 = dterm whereami kid ns denv t2 in
      let dt3 = dterm whereami kid ns denv t3 in
  ...
  | Uast.Tinfix (t1, op1, t23) ->
      ...
      let rec chain _ de1 op1 t23 =
        ...
        | { term_desc = Uast.Tinfix (t2, op2, t3); term_loc = loc23 } ->
            let de2 = dterm whereami kid ns denv t2 in
        ...
        | _ -> apply de1 op1 (dterm whereami kid ns denv t23)
      in
      chain loc (dterm whereami kid ns denv t1) op1 t23
  ...
  | Uast.Tlambda (pl, t, pty) ->
      ...
      let dt =
        match pty with
        | Some pty -> dterm_expected dt (dty_of_pty ns pty)
        | _ -> dt
      in
  ...
  | Uast.Tcase (t, ptl) ->
      ...
      let branch (p, g, t) =
        ...
        let dg =
          match g with
          | None -> None
          | Some g -> Some (dterm whereami kid ns denv g)
        in
        (dp, dg, dt)
      in
...
let dterm whereami kid ns env t =
  let denv = Mstr.map (fun vs -> dty_of_ty vs.vs_ty) env in
  dterm whereami kid ns denv t

let term_with_unify whereami kid ty ns env t =
  let dt = dterm whereami kid ns env t in
  dterm_unify dt (dty_of_ty ty);
  term env dt

let fmla whereami kid ns env t =
  let dt = dterm whereami kid ns env t in
...
let process_type_spec kid ns ty spec =
  ...
  let aux = function
    | vs, xs ->
        ...
        (self_vs, List.map (fmla Invariant kid ns env) xs)
  in
...
let type_type_declaration path kid ns r tdl =
  let add_new tdm td =
...
let process_sig_type path ~loc ?(ghost = Nonghost) kid ns r tdl =
  let r = rec_flag r in
  let tdl = type_type_declaration path kid ns r tdl in
...
let process_val_spec kid ns id args ret vs =
  ...
  let pre = List.map (fmla Requires kid ns env) vs.sp_pre in
  let checks = List.map (fmla Checks kid ns env) vs.sp_checks in
  let wr =
    List.map (fun t -> dterm Modifies kid ns env t |> term env) vs.sp_writes
  in
  let cs =
    List.map (fun t -> dterm Consumes kid ns env t |> term env) vs.sp_consumes
  in
  ...
  let process_xpost (loc, exn) =
    ...
    let process mxs (q, pt) =
      ...
      | Some (p, t) ->
          ...
          let t = fmla Raises kid ns env t in
  ...
  in
  let post = List.map (fmla Ensures kid ns env) vs.sp_post in
...
let process_val path ~loc ?(ghost = Nonghost) kid ns vd =
...
let process_function path kid ns f =
  ...
  let def =
    Option.map (term_with_unify Function_or_predicate kid f_ty ns env) f.fun_def
  in

  let spec =
    Option.map
      (fun (spec : Uast.fun_spec) ->
        let req = List.map (fmla Requires kid ns env) spec.fun_req in
        let ens = List.map (fmla Ensures kid ns env) spec.fun_ens in
        let variant =
          List.map
            (term_with_unify Variant kid ty_integer ns env)
            spec.fun_variant
        in
...
let process_axiom path loc kid ns a =
  let id = Ident.of_preid ~path a.Uast.ax_name in
  let t = fmla Axiom kid ns Mstr.empty a.Uast.ax_term in
...
and process_modtype path penv muc umty =
  ...
  | Mod_with (umty2, cl) ->
      ...
      let process_constraint (muc, cl) c =
        ...
        | Wtype (lid, tyd) ->
            let tdl =
              type_type_declaration path muc.muc_kid ns_init Nonrecursive
                [ tyd ]
            in
...
and process_sig_item path penv muc { sdesc; sloc } =
  let process_sig_item si muc =
    ...
    match si with
    | Uast.Sig_type (r, tdl) ->
        (muc, process_sig_type path ~loc:sloc kid ns r tdl)
    | Uast.Sig_val vd -> (muc, process_val path ~loc:sloc kid ns vd)
    ...
    | Uast.Sig_function f -> (muc, process_function path kid ns f)
    | Uast.Sig_axiom a -> (muc, process_axiom path sloc kid ns a)
    | Uast.Sig_ghost_type (r, tdl) ->
        (muc, process_sig_type path ~loc:sloc ~ghost:Ghost kid ns r tdl)
    | Uast.Sig_ghost_val vd ->
        (muc, process_val path ~loc:sloc ~ghost:Ghost kid ns vd)
